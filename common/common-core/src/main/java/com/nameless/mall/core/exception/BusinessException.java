...
/**
 * 可預期的業務邏輯錯誤，推薦用 ResultCodeEnum 建構。
...
public final class BusinessException extends RuntimeException {
    ...
    /**
     * 【已棄用】使用數字 code 和 message 建構業務例外。
     * <p>
     * 此建構子僅供舊代碼相容，新代碼請使用 {@link #BusinessException(ResultCodeEnum)}。
     *
     * @param legacyCode 舊版數字錯誤碼 (會被轉為字串)
     * @param message    錯誤訊息
     * @deprecated 請使用 {@link #BusinessException(ResultCodeEnum)} 代替
     */
    @Deprecated
    public BusinessException(int legacyCode, String message) {
        super(message);
        this.resultCode = null;
        this.code = String.valueOf(legacyCode);
        this.httpStatus = legacyCode >= 100 && legacyCode < 600 ? legacyCode : 500;
