...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Result<?>> handleBusinessException(BusinessException ex) {
        ...
        if (ex.getResultCode() != null) {
            // 新版: 使用 ResultCodeEnum 建構的 exception
            resultBody = Result.fail(ex.getResultCode(), ex.getMessage());
        } else {
            // 相容舊版: 使用 (int, String) 建構的 exception
            resultBody = Result.fail(ex.getCode(), ex.getMessage());
        }
    ...
    public ResponseEntity<Result<?>> handleMethodNotSupportedException(HttpRequestMethodNotSupportedException ex) {
        String message = "不支援 " + ex.getMethod() + " 方法";
        log.warn("【HTTP 方法不支援】{}", message);
    ...
    public ResponseEntity<Result<?>> handleFeignException(FeignException ex) {
        ...
        try {
            ...
            if (content != null && !content.isEmpty()) {
                // 2. 使用 ObjectMapper 安全解析 JSON，避免手動截取的脆弱性
                try {
