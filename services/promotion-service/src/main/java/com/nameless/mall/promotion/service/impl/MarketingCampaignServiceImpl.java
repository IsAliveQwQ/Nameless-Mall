...
public class MarketingCampaignServiceImpl extends ServiceImpl<MarketingCampaignMapper, MarketingCampaign>
    ...
    public List<MarketingCampaign> getActiveCampaigns() {
        ...
        // 2. 動態計算每個活動的即時狀態
        campaigns.forEach(c -> c.setStatus(computeStatus(c, now)));
        return campaigns;
    }
    ...
    @Override
    public MarketingCampaign getByCode(String code) {
        return this.getOne(new LambdaQueryWrapper<MarketingCampaign>().eq(MarketingCampaign::getCode, code));
    }
}
