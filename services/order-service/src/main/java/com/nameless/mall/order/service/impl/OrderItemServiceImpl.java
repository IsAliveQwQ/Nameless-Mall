...
public class OrderItemServiceImpl extends ServiceImpl<OrderItemMapper, OrderItem> implements OrderItemService {
    ...
    @Override
    public List<OrderItem> createFromCartItems(Order order, List<CartItemDTO> cartItems,
            Map<Long, ProductPriceResultDTO> pricingMap) {
        // 1. 呼叫純內存構建方法
        List<OrderItem> orderItems = buildOrderItems(order.getOrderSn(), cartItems, pricingMap);

        // 2. 回填 OrderId (因舊介面有傳入 Order)
