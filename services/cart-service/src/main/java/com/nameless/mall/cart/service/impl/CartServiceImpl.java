...
public class CartServiceImpl implements CartService {
     ...
     */
    @Override
    public CartVO addToCart(Long variantId, Integer quantity) {
        log.info("【購物車服務】處理加入商品請求: variantId={}, quantity={}", variantId, quantity);
        ...
        executeWithLock(() -> {
            try {
                ...
                if (StringUtils.hasText(json)) {
                    ...
                    // [Fix] originalPrice 必須是 MSRP（定價），而非當前售價
        ...
        if (priceResults != null && priceResults.isSuccess() && priceResults.getData() != null) {
            ...
            for (CartItemDTO item : items) {
                ...
                if (best != null && best.getFinalPrice() != null) {
                    // [Fix] 用計價引擎回傳的 MSRP 校正 originalPrice，確保折扣基準正確
            ...
            log.warn("【購物車計價】促銷服務不可用，啟動平滑降級 (Safe Degradation).");
            for (CartItemDTO item : items) {
     ...
     */
    @Override
    public CartVO updateItemQuantity(Long variantId, Integer quantity) {
        if (quantity <= 0)
     ...
     */
    @Override
    public CartVO removeItem(Long... variantIds) {
        // 鎖內僅做 Redis 刪除
        executeWithLock(() -> {
            BoundHashOperations<String, Object, Object> cartOps = getCartOps();
    ...
    public void clearCart() {
        executeWithLock(() -> {
            String cartKey = CART_PREFIX + getCurrentUserId();
    ...
    @Override
    public List<CartItemDTO> getCartItemsByIds(List<Long> variantIds) {
        BoundHashOperations<String, Object, Object> cartOps = getCartOps();
        List<Object> cartItemJsonList = cartOps
                .multiGet(variantIds.stream().map(String::valueOf).collect(Collectors.toList()));

        if (CollectionUtils.isEmpty(cartItemJsonList)) {
            log.warn("【購物車診斷】內部查詢查無商品: userId={}, variantIds={}", getCurrentUserId(), variantIds);
            return List.of();
        }
    ...
    @Override
    public void clearCartItems(List<Long> variantIds) {
        removeItem(variantIds.toArray(new Long[0]));
    }
    ...
    private String getCurrentUserId() {
        ...
        if (authentication == null || !authentication.isAuthenticated()
            ...
            throw new BusinessException(ResultCodeEnum.UNAUTHORIZED, "存取購物車需先進行登入");
        }
     ...
     */
    private <T> T executeWithLock(java.util.function.Supplier<T> logic) {
